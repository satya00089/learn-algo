...
import { useQuickSortPlayground } from '../hooks/useQuickSortPlayground'
import { drawArray } from '../visualizers/sortingVisualizer'

/**
 * Quick Sort Playground
...
export function QuickSortPlayground() {
  ...
  useEffect(() => {
    ...
    engineRef.current = new QuickSortEngine(initialArray)
  ...
  const getPhaseDescription = () => {
    ...
    switch (engineState.stepPhase) {
      case 'selecting':
        return '🎯 Selecting pivot'
      case 'comparing':
        return '🔍 Comparing with pivot'
      case 'swapping':
        return '🔄 Swapping elements'
      case 'pivoting':
        return '📍 Placing pivot'
      case 'recursing':
        return '↩️ Creating sub-partitions'
      default:
        return ''
    }
  }
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex-1 flex items-center">
            <Breadcrumbs />
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Quick Sort</h1>
          </div>
        ...
        <div className="flex-1 grid lg:grid-cols-4 gap-3 overflow-hidden">
          ...
          <div className="space-y-3 overflow-y-auto min-h-0 pr-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-200 dark:[&::-webkit-scrollbar-track]:bg-gray-800 [&::-webkit-scrollbar-thumb]:bg-gray-400 dark:[&::-webkit-scrollbar-thumb]:bg-gray-600 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-gray-500 dark:[&::-webkit-scrollbar-thumb]:hover:bg-gray-500">
            ...
            {engineState && (
              <ControlGroup title="Current State">
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                  <span className="text-gray-600 dark:text-gray-400">Partitions Left:</span>
                  <span className="font-semibold text-right">
                    {engineState.partitionStack.length}
                  </span>
            ...
            {/* Algorithm Info */}
            <ControlGroup title="About Quick Sort">
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  ...
                  <li>Worst: O(n²) - unbalanced partitions</li>
                </ul>
                ...
                <div className="space-y-2 text-xs">
                  ...
                  {!engineState.isSorted && (
                    <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded border border-green-200 dark:border-green-800">
                      ...
                      {engineState.currentPartition && (
                        <div className="text-green-700 dark:text-green-300 text-[10px] mt-1">
                          Partition: [{engineState.currentPartition.low},{' '}
                          {engineState.currentPartition.high}]
                          {engineState.pivotIndex !== null &&
                            ` | Pivot: ${engineState.array[engineState.pivotIndex].value}`}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="bg-purple-50 dark:bg-purple-900/20 p-2 rounded">
                    ...
                    <ol className="list-decimal list-inside space-y-1 text-gray-700 dark:text-gray-300 text-[10px]">
                      <li>Select pivot element</li>
                      <li>Partition array around pivot</li>
                      <li>Place pivot in correct position</li>
            ...
            <ControlGroup title="How It Works">
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                <p>
                  Quick Sort uses a divide-and-conquer strategy by selecting a pivot element and
                  partitioning the array so that elements smaller than the pivot come before it and
                  larger elements come after.
                </p>
                <p>
                ...
                <div className="mt-2 p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded">
                  <p className="text-yellow-800 dark:text-yellow-200 font-semibold text-[10px]">
                    💡 Tip: Watch how the pivot gets placed in its final sorted position after each
                    partition!
                  </p>
                </div>
              </div>
          ...
          >
            ...
            {isRelatedOpen && (
              <div className="px-4 pb-4 pt-2 border-t border-gray-200 dark:border-gray-700 max-h-[60vh] overflow-y-auto">
                <RelatedAlgorithms route="quick-sort" type="dsa" compact />
              </div>
            )}
          </div>
        </div>
        ...
        <TheoryModal
          ...
          theoryFile="/theory/dsa/quick-sort.md"
