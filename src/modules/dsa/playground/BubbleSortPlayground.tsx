...
import { useBubbleSortPlayground } from '../hooks/useBubbleSortPlayground'
 ...
 */
export function BubbleSortPlayground() {
  ...
  const {
    ...
    setIsDebugMode,
    generateRandomArray,
  } = useBubbleSortPlayground()

  const engineRef = useRef<BubbleSortEngine | null>(null)
  const [engineState, setEngineState] = useState<ReturnType<BubbleSortEngine['getState']> | null>(
    null
  )
  ...
  useEffect(() => {
    ...
    engineRef.current = new BubbleSortEngine(initialArray)
  ...
  // Watch for swaps and trigger haptic feedback
  useEffect(() => {
    if (engineState && engineState.swaps > previousSwapCountRef.current) {
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 grid lg:grid-cols-4 gap-3 overflow-hidden">
          ...
          <div className="space-y-3 overflow-y-auto min-h-0 pr-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-200 dark:[&::-webkit-scrollbar-track]:bg-gray-800 [&::-webkit-scrollbar-thumb]:bg-gray-400 dark:[&::-webkit-scrollbar-thumb]:bg-gray-600 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-gray-500 dark:[&::-webkit-scrollbar-thumb]:hover:bg-gray-500">
            ...
            {engineState && (
              <ControlGroup title="Current State">
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                  ...
                  <span className="font-semibold text-right">{engineState.currentPass + 1}</span>
            ...
            {/* Algorithm Info */}
            <ControlGroup title="About Bubble Sort">
                  ...
                  {!engineState.isSorted && (
                    <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded border border-green-200 dark:border-green-800">
                      ...
                      <div className="text-green-900 dark:text-green-100 text-[10px] font-semibold">
                        {engineState.stepPhase === 'comparing' && '🔍 Comparing elements'}
                        {engineState.stepPhase === 'swapping' && '🔄 Swapping elements'}
                        {engineState.stepPhase === 'moving' && '➡️ Moving to next pair'}
                      </div>
                      <div className="text-green-700 dark:text-green-300 text-[10px] mt-1">
                        Index: {engineState.currentIndex} | Pass: {engineState.currentPass + 1}
                      </div>
                    </div>
                  )}
            ...
            <ControlGroup title="How It Works">
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                ...
                <p>
                  ...
                  of the list (beginning) while larger elements sink to the bottom (end).
                </p>
                <div className="mt-2 p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded">
                  <p className="text-yellow-800 dark:text-yellow-200 font-semibold text-[10px]">
                    💡 Tip: Watch how the largest unsorted element moves to its correct position in
                    each pass!
                  </p>
                </div>
              </div>
          ...
          >
            ...
            {isRelatedOpen && (
              <div className="px-4 pb-4 pt-2 border-t border-gray-200 dark:border-gray-700 max-h-[60vh] overflow-y-auto">
                <RelatedAlgorithms route="bubble-sort" type="dsa" compact />
              </div>
            )}
          </div>
        </div>
        ...
        <TheoryModal
          ...
          theoryFile="/theory/dsa/bubble-sort.md"
