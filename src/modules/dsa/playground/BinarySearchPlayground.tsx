...
export function BinarySearchPlayground() {
  ...
  const engineRef = useRef<BinarySearchEngine | null>(null)
  const [engineState, setEngineState] = useState<ReturnType<BinarySearchEngine['getState']> | null>(
    null
  )
  ...
  // Initialize engine with random sorted array (only on arraySize change)
  useEffect(() => {
    const initialArray = generateSortedArray(arraySize)
    const initialTarget = target // Capture current target value
  ...
  useEffect(() => {
    if (engineRef.current) {
      engineRef.current.updateTarget(target)
      setEngineState(engineRef.current.getState())
    }
  }, [target])
  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D) => {
      ...
      ctx.textAlign = 'left'
      ctx.fillText(`🎯 Target: ${engineState.target}`, padding.left, 30)
      ctx.restore()

      // Draw array bars
      drawBinarySearchArray(ctx, engineState.array, engineState.target, {
        canvasWidth: width,
        canvasHeight: height,
      ...
      if (engineState.currentStep >= 0 && engineState.steps[engineState.currentStep]) {
        const currentStep = engineState.steps[engineState.currentStep]
        ctx.save()
        ctx.fillStyle = '#1f2937'
        ctx.font = '14px sans-serif'
        ctx.textAlign = 'center'
        ctx.fillText(currentStep.message, width / 2, height - 20)
        ctx.restore()
      }
    },
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Binary Search</h1>
          </div>
          <ThemeToggle />
        </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    max={100}
                    step={1}
                    onChange={(e) => setTarget(Number.parseInt(e.target.value) || 50)}
                    className="w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                </div>
            ...
            <div className="flex-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 flex flex-col min-h-0">
              ...
              <div className="flex items-center justify-center gap-6 pt-3 border-t border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-purple-400 border border-gray-300 dark:border-gray-600"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-300">Current Mid</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-blue-400 border border-gray-300 dark:border-gray-600"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-300">Search Range</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-gray-200 border border-gray-300 dark:border-gray-600"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-300">Excluded</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-green-400 border border-gray-300 dark:border-gray-600"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-300">Found</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 overflow-y-auto min-h-0 pr-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-200 dark:[&::-webkit-scrollbar-track]:bg-gray-800 [&::-webkit-scrollbar-thumb]:bg-gray-400 dark:[&::-webkit-scrollbar-thumb]:bg-gray-600 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-gray-500 dark:[&::-webkit-scrollbar-thumb]:hover:bg-gray-500">
              ...
              <ControlGroup title="Current State">
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                  ...
                  <span className="font-semibold text-right">
                    {engineState.currentStep + 1} / {engineState.steps.length}
                  </span>
                  ...
                  {engineState.currentStep >= 0 && (
                    <>
                      ...
                      <span className="font-semibold text-right">{engineState.left}</span>

                      <span className="text-gray-600 dark:text-gray-400">Right:</span>
                      <span className="font-semibold text-right">{engineState.right}</span>

                      <span className="text-gray-600 dark:text-gray-400">Mid:</span>
                      <span className="font-semibold text-right">{engineState.mid}</span>
                    </>
                  )}
                  ...
                  <span className="font-semibold text-right">
                    {engineState.isComplete
                      ? engineState.found
                        ? `✓ Found at ${engineState.foundIndex}`
                        : '✗ Not Found'
                      : '⟳ Searching'}
                  </span>
                </div>
            ...
            <ControlGroup title="About Binary Search">
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                ...
                <p className="mt-2">
                  <strong className="text-gray-800 dark:text-white">Requirement:</strong> Sorted
                  Array
                </p>
              </div>
            </ControlGroup>
            ...
            {/* Debug Mode Info */}
            {isDebugMode && engineState && engineState.currentStep >= 0 && (
                ...
                <div className="space-y-2 text-xs">
                  ...
                  {!engineState.isComplete && (
                    <div className="bg-purple-50 dark:bg-purple-900/20 p-2 rounded border border-purple-200 dark:border-purple-800">
                      ...
                      <div className="text-purple-900 dark:text-purple-100 text-[10px] font-semibold">
                        {engineState.stepPhase === 'comparing' && '🔍 Comparing with mid element'}
                        {engineState.stepPhase === 'found' && '✓ Target found!'}
                        {engineState.stepPhase === 'notFound' && '✗ Target not in array'}
                        {engineState.stepPhase === 'updateBounds' && '➡️ Narrowing search range'}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-2 rounded">
                    ...
                    <ol className="list-decimal list-inside space-y-1 text-gray-700 dark:text-gray-300 text-[10px]">
                      ...
                      <li>Compare target with middle</li>
                      <li>If equal, search complete</li>
                      <li>If less, search left half</li>
                      <li>If greater, search right half</li>
                    </ol>
                  </div>
                  ...
                  {engineState.currentStep >= 0 && (
                    <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded">
                      ...
                      <div className="text-[10px] text-gray-700 dark:text-gray-300">
                        {engineState.steps[engineState.currentStep].message}
                      </div>
                    </div>
                  )}
                </div>
            ...
            <ControlGroup title="How It Works">
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                <p>
                  Binary Search efficiently finds a target value in a sorted array by repeatedly
                  dividing the search space in half.
                </p>
                <p>
                  Each comparison eliminates half of the remaining elements, resulting in
                  logarithmic time complexity.
                </p>
                <div className="mt-2 p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded">
                  <p className="text-yellow-800 dark:text-yellow-200 font-semibold text-[10px]">
                    💡 Tip: The array must be sorted for binary search to work correctly. Watch how
                    the search range shrinks by half each step!
                  </p>
                </div>
              </div>
            </ControlGroup>
