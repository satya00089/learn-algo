...
/**
 * Queue Playground
 * Visualizes queue operations (FIFO - First In First Out)
 */
export function QueuePlayground() {
  ...
  // Initialize engine
  useEffect(() => {
    const initialQueue = generateInitialQueue(maxSize)
    engineRef.current = new QueueEngine(maxSize, initialQueue)
    setEngineState(engineRef.current.getState())
  }, [maxSize, generateInitialQueue])

  // Draw function - horizontal queue visualization
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D) => {
      ...
      if (!engineState || engineState.queue.length === 0) {
        // Draw empty queue message
        ctx.font = '20px Arial'
        ctx.fillStyle = '#9ca3af'
        ctx.textAlign = 'center'
        ctx.fillText('Queue is Empty', width / 2, height / 2)
        return
      }
      ...
      const gap = 10
      const totalWidth = engineState.queue.length * (elementWidth + gap) - gap
      const startX = width / 2 - totalWidth / 2
      const startY = height / 2 - elementHeight / 2

      // Draw queue elements from left (front) to right (rear)
      engineState.queue.forEach((element: ArrayElement, index: number) => {
        ...
        switch (element.state) {
          case 'sorted':
            fillColor = '#10b981' // green (front)
            strokeColor = '#059669'
            break
          case 'comparing':
            fillColor = '#f59e0b' // amber (rear)
            strokeColor = '#d97706'
            break
          case 'swapping':
        ...
        ctx.fillText(element.value.toString(), x + elementWidth / 2, y + elementHeight / 2)

        // Draw index label
        ctx.font = '12px Arial'
        ctx.fillStyle = '#6b7280'
        ctx.textAlign = 'center'
        ctx.fillText(`[${index}]`, x + elementWidth / 2, y + elementHeight + 15)
      })

      // Draw "FRONT" indicator
      if (engineState.queue.length > 0 && engineState.frontIndex >= 0) {
        const frontX = startX + engineState.frontIndex * (elementWidth + gap)
        const frontY = startY

        ctx.font = 'bold 14px Arial'
        ctx.fillStyle = '#10b981'
        ctx.textAlign = 'center'
        ctx.fillText('FRONT', frontX + elementWidth / 2, frontY - 15)

        // Arrow
        ctx.beginPath()
        ctx.moveTo(frontX + elementWidth / 2, frontY - 5)
        ctx.lineTo(frontX + elementWidth / 2 - 5, frontY - 10)
        ctx.moveTo(frontX + elementWidth / 2, frontY - 5)
        ctx.lineTo(frontX + elementWidth / 2 + 5, frontY - 10)
      ...
      // Draw "REAR" indicator
      if (engineState.queue.length > 0 && engineState.rearIndex >= 0) {
        const rearX = startX + engineState.rearIndex * (elementWidth + gap)
        const rearY = startY

        ctx.font = 'bold 14px Arial'
        ctx.fillStyle = '#f59e0b'
        ctx.textAlign = 'center'
        ctx.fillText('REAR', rearX + elementWidth / 2, rearY + elementHeight + 35)

        // Arrow
        ctx.beginPath()
        ctx.moveTo(rearX + elementWidth / 2, rearY + elementHeight + 25)
        ctx.lineTo(rearX + elementWidth / 2 - 5, rearY + elementHeight + 20)
        ctx.moveTo(rearX + elementWidth / 2, rearY + elementHeight + 25)
        ctx.lineTo(rearX + elementWidth / 2 + 5, rearY + elementHeight + 20)
      ...
      ctx.fillStyle = '#6b7280'
      ctx.textAlign = 'center'
      ctx.fillText('→ Enqueue Direction', width / 2, height - padding.bottom + 20)
    },
  ...
  const handleGenerateNew = () => {
    const newQueue = generateInitialQueue(maxSize)
    if (engineRef.current) {
      engineRef.current.updateQueue(newQueue)
      setEngineState(engineRef.current.getState())
    }
    setIsPlaying(false)
    if (playIntervalRef.current) {
  ...
  const handleEnqueue = () => {
    ...
    engineRef.current.startEnqueue(value)
    setEngineState(engineRef.current.getState())
    setEnqueueValue('')
  }
  ...
  const handleDequeue = () => {
    if (!engineRef.current) return
    engineRef.current.startDequeue()
    setEngineState(engineRef.current.getState())
  }
      ...
      <div className="h-full flex flex-col">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Queue (FIFO)</h1>
          </div>
          <ThemeToggle />
        </div>
        ...
        <div className="flex-1 grid lg:grid-cols-4 gap-3 overflow-hidden">
          <div className="lg:col-span-3 flex flex-col space-y-3 min-h-0 overflow-visible">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Tooltip text="Generate New Queue">
                  <button
            ...
            <div className="flex-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 flex flex-col min-h-0">
              ...
              <div className="flex items-center justify-center gap-6 pt-3 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600 dark:text-gray-300">Front</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-amber-500 border border-gray-300 dark:border-gray-600"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-300">Rear</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 overflow-y-auto min-h-0 pr-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-200 dark:[&::-webkit-scrollbar-track]:bg-gray-800 [&::-webkit-scrollbar-thumb]:bg-gray-400 dark:[&::-webkit-scrollbar-thumb]:bg-gray-600 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-gray-500 dark:[&::-webkit-scrollbar-thumb]:hover:bg-gray-500">
            ...
            {engineState && (
              <ControlGroup title="Current State">
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                  ...
                  <span className="font-semibold text-right">{engineState.queue.length}</span>
                  ...
                  <span className="text-gray-600 dark:text-gray-400">Front Index:</span>
                  <span className="font-semibold text-right">{engineState.frontIndex}</span>

                  <span className="text-gray-600 dark:text-gray-400">Rear Index:</span>
                  <span className="font-semibold text-right">{engineState.rearIndex}</span>

                  <span className="text-gray-600 dark:text-gray-400">Is Empty:</span>
                  <span className="font-semibold text-right">
                    {engineState.queue.length === 0 ? 'Yes' : 'No'}
                  </span>
            ...
            {/* Enqueue Operation */}
            <ControlGroup title="Enqueue (Add to Rear)">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={!enqueueValue || isPlaying}
            ...
            {/* Dequeue Operation */}
            <ControlGroup title="Dequeue (Remove from Front)">
              <div className="space-y-2">
                <p className="text-xs text-gray-600 dark:text-gray-300">
                ...
                <button
                  ...
                  disabled={isPlaying || !engineState || engineState.queue.length === 0}
            ...
            {/* Peek Operation */}
            <ControlGroup title="Peek (View Front)">
              <div className="space-y-2">
                <p className="text-xs text-gray-600 dark:text-gray-300">
            ...
            <ControlGroup title="Query Operations">
              <div className="space-y-2">
                ...
                >
                  <FaRulerHorizontal size={10} /> Get Size
                </button>
                <button
                  onClick={handleClear}
                  disabled={isPlaying || !engineState || engineState.queue.length === 0}
                ...
                <div className="space-y-2 text-xs">
                  <div className="bg-gray-50 dark:bg-gray-900/50 p-2 rounded">
                    ...
                    <div className="font-mono text-[10px] text-gray-600 dark:text-gray-400 break-words">
                      [{engineState.queue.map((el) => el.value).join(', ')}]
                    </div>
                  </div>
            ...
            {/* About Queue */}
            <ControlGroup title="About Queue">
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                <p>
                  <strong className="text-gray-800 dark:text-white">FIFO Principle:</strong> First
                  In, First Out - the oldest element is removed first.
                </p>
                <p className="mt-2">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li>Enqueue: O(1)</li>
                  <li>Dequeue: O(1) with circular array</li>
                  <li>Peek: O(1)</li>
                </ul>
                <p className="mt-2">
                  <strong className="text-gray-800 dark:text-white">Use Cases:</strong> Task
                  scheduling, print queue, breadth-first search, message queuing
                </p>
              </div>
            </ControlGroup>
          </div>
