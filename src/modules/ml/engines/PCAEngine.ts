...
export class PCAEngine {
  ...
  private powerIteration(matrix: number[][], deflationCount: number): { eigenvalue: number; eigenvector: number[] } {
  ...
  private transformData(): void {
    ...
    for (const point of this.state.points) {
      ...
      let transformedX = 0, transformedY = 0, transformedZ = 0

      if (this.state.components.length > 0) {
        const pc1 = this.state.components[0]
        if (hasZ && pc1.eigenvector.length === 3) {
          transformedX = centered.x * pc1.eigenvector[0] + centered.y * pc1.eigenvector[1] + centered.z * pc1.eigenvector[2]
        } else {
      ...
      if (this.state.components.length > 1) {
        ...
        if (hasZ && pc2.eigenvector.length === 3) {
          transformedY = centered.x * pc2.eigenvector[0] + centered.y * pc2.eigenvector[1] + centered.z * pc2.eigenvector[2]
        } else {
      ...
      if (this.state.components.length > 2 && hasZ) {
        ...
        transformedZ = centered.x * pc3.eigenvector[0] + centered.y * pc3.eigenvector[1] + centered.z * pc3.eigenvector[2]
      }
